...
export default function useCategoriesList() {
  ...
  // Table Handlers
  const tableColumns = [
    { key: 'category_name', sortable: true, label: 'Name' },
    { key: 'actions' },
  ]
  ...
  const fetchRecords = (ctx, callback) => {
    store
      .dispatch('Bcategory/fetchRecords', {
        q: searchQuery.value,
      ...
      .then(response => {
        const { Bcategory } = response.data.data
        console.log(response.data.data)
        callback(Bcategory.data)
        totalRecords.value = Bcategory.total
      })
      .catch(() => {
        toast({
