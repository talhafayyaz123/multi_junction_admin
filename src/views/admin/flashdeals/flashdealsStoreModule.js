...
export default {
  ...
  getters: {
    records(state) {
      return state.records.flashdeals.data
    },
    getRecordById: state => id => state.records.flashdeals.data.find(record => record.id === id),
    record(state) {
      return state.record
    },
  },
  ...
  actions: {
    ...
    fetchRecords(ctx, queryParams) {
      return new Promise((resolve, reject) => {
        axios
          .get('/flashdeals/fetchRecords', { params: queryParams })
          .then(response => {
    ...
    fetchRecord(ctx, id) {
      return new Promise((resolve, reject) => {
        axios
          .get(`/flashdeals/fetchRecords/${id}`)
